...
def build_unet(unet_type,
    ...
    elif unet_type == 'DualU':

        model = DualUNet(ch_in=ch_in,
                         ch_out=ch_out,
                         pool_method=pool_method,
                         filters=filters,
                         act_fun=act_fun,
...
class DualUNet(nn.Module):
    """ Implementation of the Dual U-net architecture with two decoder paths and feature fusion layers.

    Changes to original architecture: Convolutions with stride 2 can be used instead of max-pooling. Dropout layers are
        removed.

    Reference: Xieli Li et al. "Dual U-Net for the Segmentation of Overlapping Glioma Nuclei". In: IEEE Access. 2019.
    ...
    def __init__(self, ch_in=1, ch_out=1, pool_method='conv', act_fun='relu', normalization='bn', filters=(64, 1024)):
        ...
        # Last 1x1 convolutions (always 1 channel: boundary or dist)
        self.decoder1Conv.append(nn.Conv2d(n_featuremaps, 1, kernel_size=1, stride=1, padding=0))
        self.decoder2Conv.append(nn.Conv2d(n_featuremaps, 1, kernel_size=1, stride=1, padding=0))

        # Feature fusion layers (after concatenation)
        self.featureFusion = nn.ModuleList()
        self.featureFusion.append(ConvBlock(ch_in=2, ch_out=64, act_fun=act_fun, normalization=normalization))
        self.featureFusion.append(nn.Conv2d(64, 1, kernel_size=1, stride=1, padding=0))
    ...
    def forward(self, x):
        ...
        # Feature fusion layers (apply sigmoid since it is also applied automatically in the loss function)
        x3 = torch.cat([torch.sigmoid(x1), x2], 1)
        for i in range(len(self.featureFusion)):
            x3 = self.featureFusion[i](x3)
